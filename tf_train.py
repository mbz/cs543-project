...
def generate_all_patches_from_indices(image_id, images_norm):
    ...
    # Assuming label 0 has the max number of patches
    maxlen = len(label_indices[0])
    
    # Loop through indices
    X = np.zeros((batch_size, patch_size, patch_size, n_channels))
    Y = np.zeros((batch_size, 2), dtype=bool)
    X_ind = 0
    for i in range(maxlen):
        for label in range(label_count):
            for rep in range(4 if label == 0 else 1):
                (x,y,z) = label_indices[label].iloc[indices[label]]
                patch = get_patch(images_norm, x, y, z, patch_size)
                X[X_ind] = patch
                Y[X_ind, min(label, 1)] = True
...
model_path = "./models/model_BvS.ckpt"
